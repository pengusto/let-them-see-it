# Let Them See It

[English](README.md) · [Deutsch](README.de.md) · [简体中文](README.zh-CN.md) · [日本語](README.ja.md)

[![Agent skill](https://img.shields.io/badge/type-agent%20skill-blue)](SKILL.md)
[![License: MIT](https://img.shields.io/badge/license-MIT-green)](LICENSE)

GitHub リポジトリの公開に向けて準備するスキルです。現状を確認し、具体的な改善案を提示して、承認された変更を実施します。

## 使い方の流れ

1. ファイルを変更せずに、ドキュメント、リポジトリの整理状況、機密情報、ライセンス、必要な検証を確認します。
2. 根拠と確認方法を添えた改善計画を優先度順に提示します。ユーザーが対象を承認または調整します。
3. 合意した変更を実施し、検証結果、未解決の項目、公開を実施したかどうかを報告します。

標準で英語、ドイツ語、簡体字中国語、日本語の README を提案します。バッジ、設定例、コミュニティ向けファイル、CI は、プロジェクトに役立つ場合に追加します。公開には許可が必要で、準備とは別の操作として扱います。

## インストールと使用

Git と、`SKILL.md` 形式のスキルに対応したエージェントが必要です。標準構成の Codex では、まだ存在しないスキルディレクトリにクローンしてください。

```sh
git clone https://github.com/pengusto/let-them-see-it.git ~/.codex/skills/let-them-see-it
```

独自のスキルディレクトリを使っている場合は、そちらにクローンしてください。スキルが表示されない場合は、新しいエージェントセッションを開始して、次のように依頼します。

```text
$let-them-see-it を使って、このリポジトリが公開できる状態か評価してください。
ファイルを変更する前に、変更案を見せてください。
```

計画を確認したら、実施してほしい項目を承認します。スキル自体に実行時パッケージや API キーは不要です。GitHub へのアクセス権やプロジェクト固有のツールは、それらを使う操作でのみ必要です。

## 対象範囲と制限

これは指示ベースのスキルであり、自動的な安全性の認証ではありません。利用できるスキャナーやエージェントの動作は環境によって異なります。未検証の項目を報告し、シークレットの値を出力しないことを求めます。ライセンスや素材の権利が不明な場合は、メンテナーの判断が必要です。翻訳は AI が作成しており、各言語に堪能な第三者による確認は行っていません。

手順は [SKILL.md](SKILL.md)、繰り返し実施できる動作確認は [TESTING.md](TESTING.md) を参照してください。

## コントリビューション

実際の動作と期待する結果を記載した Issue、または変更範囲を絞ったプルリクエストを送ってください。4 言語の README の内容を揃えてください。報告には認証情報や非公開リポジトリの内容を含めないでください。

## ライセンス

[MIT](LICENSE) © 2026 Pengusto。
