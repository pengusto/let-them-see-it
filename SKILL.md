...
- Languages: propose English `README.md`, German `README.de.md`, Simplified Chinese `README.zh-CN.md`, Japanese `README.ja.md`, Spanish `README.es.md`, Korean `README.ko.md`, Brazilian Portuguese `README.pt-BR.md`, and French `README.fr.md` by default, with reciprocal language links and equivalent content. Before implementation, explicitly list all planned languages and filenames in the evaluation, explain that translations will be maintained together, and let the user reduce or change the selection through the scope approval. Do this even when using the defaults; do not silently generate eight languages. Respect existing locale conventions and user preferences. Keep code, commands, identifiers, and paths consistent; disclose machine translation when no fluent review was performed.
...
- GitHub surfaces: evaluate Sponsors (`.github/FUNDING.yml`), Discussions, Releases, `CITATION.cff`, repository topics/description, social preview, and Insights traffic when relevant. For project analysis, optionally propose [Star History](https://github.com/star-history/star-history) to inspect star growth, momentum, unusual spikes, or several repositories together. Verify its current public status and MIT license before recommending it, check that the chart currently loads, and label the result as a star signal rather than proof of users or quality. Star-history charts have had intermittent GitHub API and endpoint failures; a broken or stale chart must be reported and never treated as evidence. Mark each surface as useful, unnecessary, or blocked by missing profile, audience, maintainer capacity, or project metadata. Do not create payment links, enable features, publish a release, or claim reach from a badge without explicit scope and evidence.
